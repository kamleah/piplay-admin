...
import { createCouponAPI, editCouponAPI, getAllMatchesApi, getAllTournamentsAPI } from "../apiFile/Service";
...
const AddCoupon = ({
    ...
    const data1 = data || [];
    const { register, handleSubmit, reset, setValue, formState, watch, control } = form;
    ...
    const options: any = [
        { label: "Pi Play", value: "piplay" },
        { label: "Facility", value: "facility" },
        { label: "Both", value: "both" },
    ];

    const couponFromValue: any = watch("coupon_from");
    ...
    const onSubmit = async (data: formModal) => {
        ...
        if (data.event_id != "") {
            ...
            data.event_id = data.event_id?.map((item: any) => item?.value)
        }
        data.facility_percentage = Number(data.facility_percentage)
        data.coupon_from = data.coupon_from.value
    ...
    useEffect(() => {
        if (edit == true) {
            let facilities = facilitydata?.map(items => {
                let facility = editdata?.facility_id?.filter(data => data == items.value)
                if (facility?.length > 0) {
                    return items
                }
            })
            facilities = facilities.filter(data => data);
            if (editdata?.facility_id[0]?.includes("All")) {
                facilities = [{ label: "All", value: "All" }, ...facilities];
            } else {
                facilities = facilities.filter(data => data);
            }
            ...
            reset({
                ...
                coupon_from: { label: editdata?.coupon_from, value: editdata?.coupon_from } as any,
                facility_percentage: editdata?.facility_percentage,
                ...
                event_id: tournaments?.filter((data: any) =>
                    editdata?.event_id.includes(data?.value)
                ).map((data: any) => ({
                    label: data?.label,
                    value: data?.value,
                })),
                sport_type: { label: editdata?.sport_type, value: editdata?.sport_type },
                gender: { label: editdata?.gender, value: editdata?.gender },
                    ...
                    <form style={{ width: '100%' }} onSubmit={handleSubmit(onSubmit)}>
                        <div className="border-bottom-light">
                            <div className="form-container-grid">
                                ...
                                <div className="input-group">
                                    <label htmlFor="coupon_from" className="form-lable">Coupon From</label>
                                    <div className="form-group">
                                        <Controller
                                            name="coupon_from"
                                            control={control}
                                ...
                                {couponFromValue?.value === "both" && (
                                    <div className="input-group">
                                        <label htmlFor="facility_percentage">Facility Percentage<span style={{ color: "red" }}>*</span></label>
                                        <div className="form-group">
                                            <input
                                                type="number"
                                                id="facility_percentage"
                                                placeholder="Facility percentage"
                                                {...register('facility_percentage', {
                                                    required: {
                                                        value: true,
                                                ...
                                                render={({ field: { onChange, value }, field }) => (
                                                    <Select
                                                        ...
                                                        onChange={(selectedOptions) => {
                                                            ...
                                                            if (isAllSelected) {
                                                                setValue('facility_id', [{ label: 'All', value: 'All' }]);
                                                            } else {
                                                                setValue('facility_id', selectedOptions);
                                                            }
                                ...
                                <div className="input-group">
                                    <label htmlFor="location">Select Gender<span style={{ color: "red" }}>*</span></label>
                                    <div className="form-group">
                                        <Controller
                                            name="gender"
                                            control={control}
