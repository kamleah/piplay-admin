...
import { createHowToAPI, editHowToAPI } from '../apiFile/Service';
...
const AddHowTo = ({ visible, onCancel, row, edit, setRow, setEdit, getAllData, }) => {
    const form = useForm({
        defaultValues: {
            title: "",
            category: {},
    ...
    const onSubmit = async (data: formModal) => {
        ...
        var response; 
        if (edit == true) {
            response = await editHowToAPI(loggedInUser, row?._id, data);
        } else {
            response = await createHowToAPI(loggedInUser, data);
        }
