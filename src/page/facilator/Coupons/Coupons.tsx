...
const Coupons = ({ matches, menuOpen, onToggle, modulePermissionsData }) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: row =>` ${row?.redeemcount}/${row?.limit}   `,
            wrap: true,
            sortable: true,
    ...
    const getCoupons = async () => {
        ...
        if (!loggedUserDetails?.roleId) {
            ...
            setData(response?.result?.result.filter(item => { return (item?.facility_id == loggedUserDetails?.facility_id || item?.facility_id == undefined) }))
        }
    }
