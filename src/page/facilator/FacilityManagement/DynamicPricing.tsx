...
import { dynamicSlotPricing, filterCourt, getFacilityApi, getFacilityByIdApi, getNewBookings } from "../../../components/apiFile/Service";
...
const NewBooking = ({ matches, menuOpen, onToggle, modulePermissionsData }) => {
    ...
    const handlePriceChange = (e, slot, index) => {
        console.log(slot);
        ...
        const newSlotData = {
            ...
            date: slot?.date,
            slot_size: moment(slot?.endTime, 'HH:mm').diff(moment(slot?.startTime, 'HH:mm'), 'minutes'),
        };

        setUpdatedSlots((prevSlots) => {
            const existingIndex = prevSlots.findIndex(
                (item) =>
                    item.facility_id === newSlotData.facility_id &&
                    item.court_id === newSlotData.court_id &&
                    item.start_time === newSlotData.start_time &&
                    item.end_time === newSlotData.end_time &&
                    item.date === newSlotData.date
            );

            if (existingIndex > -1) {
                const updatedArray = [...prevSlots];
                updatedArray[existingIndex] = newSlotData;
                return updatedArray;
            } else {
                return [...prevSlots, newSlotData];
            }
        });
    };

    const handleSavePricing = async () => {
        const response = await dynamicSlotPricing(loggedInUser, updatedSlots);
        if (response.statusCode === 0) {
            getBookings();
            setEdit(false);
            setUpdatedSlots([]);
            toast(<ToastMessage body={"Pricing Updated Successfully"} type="success" />, {
                position: "top-right",
                autoClose: 5000,
            ...
            toast(<ToastMessage body={"Failed to Update Pricing"} type="error" />, {
                position: "top-right",
                autoClose: 5000,
                hideProgressBar: true,
                closeOnClick: true,
                ...
                <Card>
                    {!matches && (
                        <Breadcrumb
                            items={[
                                ...
                                {
                                    title: "Dynamic Pricing Slotwise",
                                },
                            ]}
                        />
                    )}
                    <div className="main-title-container">
                        <div className="title-add-mobile">
                            <h5 className="main-content-title">Dynamic Pricing Slotwise</h5>
                            {matches && modulePermissionsData?.edit &&
                                <>
                                    {edit ?
                                        <>
                                            {updatedSlots && updatedSlots.length > 0 &&
                                                <Button className="pi-btn-primary" key="cancel" onClick={() => handleSavePricing()}>
                                                    Save Edit
                                                </Button>}
                                            <Button className="pi-btn-secondary" key="cancel" onClick={() => { setEdit(false); setUpdatedSlots([]); }}>
                                                Cancel
                                            </Button>
                                        </>
                                        :
                                        <Button className="pi-btn-primary" key="cancel" onClick={() => setEdit(true)}>
                                            Edit Pricing
                                        </Button>}
                                </>
                            }
