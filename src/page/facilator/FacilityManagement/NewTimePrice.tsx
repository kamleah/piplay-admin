...
import { deleteCourt, deleteCourtTimePrice, filterCourt, filterCourtTimePrice, getAllCoaches, getAllCourt, getAllCourtTimePrice, getAllNewCourtTimePrice, getFacilityApi, getFacilityByIdApi } from "../../../components/apiFile/Service";
...
const NewTimePrice = ({ matches, menuOpen, onToggle, modulePermissionsData }) => {

    const moduleName = "Court Timing & Pricing"
    ...
    let facility_loc = watch("address");
    ...
    const columns = [
        ...
        {
            name: 'Break Time',
            selector: row => row?.break_start_time + ' - ' + row?.break_end_time,
            sortable: true,
            wrap: true,
        },
        {
            name: 'Slot size',
            selector: row => row?.slot_size + ' min',
            sortable: true,
            wrap: true,
        },
        {
        ...
        {
            name: 'Active Days',
            selector: row => formatWeekdays(row?.active_days),
            sortable: true,
            wrap: true,
        },
        {
            ...
            sortable: true,
            wrap: true, cell: row =>
                <div className='playerContainer max-width-container' style={{ marginRight: '30px' }}>
                    <StatusLabel
                        status={
                            // (moment() >= (moment(row.active_start_date)) && (moment() <= moment(row.active_end_date))) ? "Active" : "Scheduled"
                            ((moment().format('yyyy-MM-DD') < moment(row.active_start_date).format('yyyy-MM-DD')) && (moment().format('yyyy-MM-DD') < moment(row.active_end_date).format('yyyy-MM-DD'))) ? "Scheduled" :
                                ((moment().format('yyyy-MM-DD') >= moment(row.active_start_date).format('yyyy-MM-DD')) && (moment().format('yyyy-MM-DD') <= moment(row.active_end_date).format('yyyy-MM-DD'))) ? "Active" : "Expired"
                        }
                    />
                </div>
            ,
        },
        {
            ...
            wrap: true,
            cell: row =>
                <div className='action-button-container' style={{ marginLeft: '10px' }}>
    ...
    const filterData = async (search, value) => {
        ...
        let response = await filterCourtTimePrice(loggedInUser,
            loggedUserDetails?.roleId ? loggedUserDetails?.facility_id : facility_id == undefined ? '' : facility_id?.value,
    ...
    const getAllData = async () => {
        let response
        if (!loggedUserDetails?.roleId) {
            response = await getAllNewCourtTimePrice(loggedInUser);
        } else {
            response = await filterCourtTimePrice(loggedInUser, loggedUserDetails?.facility_id, "", "", "",);
        }
        setData(response?.result);
    }
            ...
            <>
                ...
                <div className="filter-form">
                    <div className={`${matches ? "filter-section  border-bottom-light" : "filter-fields"}`}>
                        ...
                        <div className="input-group">
                            <label htmlFor="address" className="form-lable">Facility Location</label>
                            <div className="form-group">
                                <input
        ...
        <Fragment>
            ...
            >
                                ...
                                <div className="add-export-btn">
                                    ...
                                    {!matches && modulePermissionsData?.add &&
                                        <Button
                                            ...
                                            onClick={() => { setEdit(false); setShowFormModal(true) }}
                                        >
                                            Add Court Timing and Pricing
                                        </Button>
                                    }
                                    {modulePermissionsData?.export && data?.length > 0 &&
                                        <Excel page={'manage_court_time_price'} importdata={data} />
                                    }
                                    {matches &&
                ...
                <TimePriceDetails
                    ...
                    name="Court Timing & Pricing Details"
                    row={rowdata}
                />
                <AddTimePrice
